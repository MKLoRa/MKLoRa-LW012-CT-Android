# LW012_CT Android SDK Guide（English）
...
**`com.moko.support.lw012ct.LoRaLW012CTMokoSupport`**
...
**`com.moko.support.lw012ct.OrderTaskAssembler`**

We assemble read data and write data to `OrderTask`, send the task to the device
through `LoRaLW012CTMokoSupport `, and receive the resopnse.
...
First of all, you should initialize the LoRaLW012CTMokoSupport.We recommend putting it in Application.
...
in LoRaLW012CTMokoSupport `AndroidManifest.xml`
...
```
LoRaLW012CTMokoSupport.getInstance().connDevice(beaconXInfo.mac);
```
...
```
LoRaLW012CTMokoSupport.getInstance().sendOrder(OrderTaskAssembler.setPassword(password));
...
```
// read lora region
LoRaLW012CTMokoSupport.getInstance().sendOrder(derTaskAssembler.getLoraRegion());
...
// get result
@Subscribe(threadMode = ThreadMode.MAIN)
public void onOrderTaskResponseEvent(OrderTaskResponseEvent event) {
...
2.`EventBus` is used in the SDK and can be modified in `LoRaLW012CTMokoSupport` if you want to use
