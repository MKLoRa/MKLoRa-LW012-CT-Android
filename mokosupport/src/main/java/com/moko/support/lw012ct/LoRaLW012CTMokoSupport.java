...
import com.moko.support.lw012ct.handler.MokoCharacteristicHandler;
...
public class LoRaLW012CTMokoSupport extends MokoBleLib {
    private HashMap<OrderCHAR, BluetoothGattCharacteristic> mCharacteristicMap;

    private static volatile LoRaLW012CTMokoSupport INSTANCE;

    private Context mContext;

    private MokoBleConfig mBleConfig;

    private LoRaLW012CTMokoSupport() {
        //no instance
    }

    public static LoRaLW012CTMokoSupport getInstance() {
        if (INSTANCE == null) {
            synchronized (LoRaLW012CTMokoSupport.class) {
                if (INSTANCE == null) {
                    INSTANCE = new LoRaLW012CTMokoSupport();
                }
            }
        }
        return INSTANCE;
